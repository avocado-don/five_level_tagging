...
　ユーザー一覧やフォロー一覧からは、各ユーザーが更新した最新リストにもアクセスできる。  
<br>
...
<br>
　(3)本番環境に投稿済みのサンプルデータ(list6)：「【小学生低学年】運動遊び」  
...
### 2-1-6. キーワード検索：リストやアイテムの表示・並び替え、各アイテムの親リストへ遷移
...
### 2-2-11. その他：ページネーション／レスポンシブデザイン／アプリ使用方法ガイドボタン
<br>
<br>
<br>
...
　51-53: timesメソッド：tableのtr(1行)内に、tdを4回繰り返し配置。タグ・スコアの存在判定は各td内で実行。  
　55: 1つのアイテムに紐づくi番目(0〜3番目)のタグが存在する場合、タグ名(タグ詳細画面へのリンク)を表示。  
<br>
...
### 4-2-1. 投稿・編集画面：タグ名入力欄と、ボタンの表示予定領域(空のdiv要素)を4組設置
...
　inputイベント1回につき、1秒待機してから検索を実行。1秒未満で次のinputイベントが発火したら(＝素早く文字入力したら)、待機中の検索を取り消し、次の検索を待機・実行。  
<br>
...
　22, 26: js_handle(span要素)：ドラッグ操作時につかめる要素。アイテム番号かアイテム名をつかむことで、1つのアイテム全体をドラッグできる。  
<br>
app/views/shared/_item_table.html.erb  
...
### 4-5-4. サンプルデータ群：同時刻に一斉保存後、更新日時の時差を拡大して上書き
...
<br>
　アイテムと中間テーブル(のスコア)は1対多。仮に、sort_linkのソートキー(関連テーブル名＋カラム名)を文法通りにitem_tags_scoreと複数形で表記した場合、並び替えボタンをクリックした時だけ、1つのアイテムに紐づく中間テーブルを全て(最大4つ)取得し、同じアイテムを重複して(最大4つ)表示してしまう。  
<br>
　そこで、並び替え用のクエリ処理をスコープとして定義し、sort_linkの第二引数から呼び出す。ソートキーは、1つの中間テーブルのスコアという意味で、item_tag_scoreと命名した(item_tagsを単数形の表記に変更)。  
<br>
