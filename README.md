...
### 4-1-1. テーブル設計：items, tags, item_tags(中間テーブルでもあり、scoreも保存するテーブル)
...
### 4-1-3. アイテム投稿・編集フォーム：タグ・スコアの入力値4つずつを、配列に格納して送信
...
### 4-1-6. フォームオブジェクト：saveメソッドを作り、配列内の値を4つのレコードに分けて保存
...
### 4-1-9. フォームオブジェクト：updateメソッドを作り、アイテム・タグ・スコアを更新
